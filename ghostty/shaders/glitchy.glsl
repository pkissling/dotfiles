...
// amount of seconds for which the glitch loop occurs (5 minutes = 300 seconds)
#define DURATION 300.
// percentage of the duration for which the glitch is triggered (much shorter)
#define AMT .003
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // analog distortion - only during glitch
    float y = uv.y * iResolution.y;
    float distortion = gnoise(vec3(0., y * .01, t * 500.)) * (glitchAmount * 1.);
    distortion *= gnoise(vec3(0., y * .02, t * 250.)) * (glitchAmount * 0.5);

    ++displayNoise;
    distortion += smoothstep(.999, 1., sin((uv.y + t * 1.6) * 2.)) * .02 * glitchAmount;
    distortion -= smoothstep(.999, 1., sin((uv.y + t) * 2.)) * .02 * glitchAmount;
    st = uv + vec2(distortion, 0.);
    // chromatic aberration - only during glitch
